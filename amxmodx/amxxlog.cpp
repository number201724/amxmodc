...
void CLog::CreateNewFile()
{
	...
	if (!fp)
	{
		ALERT("[AMXX] Unexpected fatal logging error. AMXX Logging disabled.\n");
...
void CLog::Reload()
{
	...
	m_LogType = atoi(get_localinfo("ext_logging", "1"));
	
	if (m_LogType < 0 || m_LogType > 3)
	{
		m_LogType = 1;
		_print_console("[AMXX] Invalid amxx_logging value; setting back to 1...");
	}
	...
	if (m_LogType == 1 || m_LogType == 2)
	{
		...
		if (m_LogType == 2)
		{
			...
			if (!pF)
			{
				...
				if (!pF)
				{
					ALERT("[AMXX] Unexpected fatal logging error (couldn't open %s for a+). AMXX Logging disabled for this map.\n", m_LogFile.c_str());
					m_LogType = 0;
					return;
				}
			}
...
void CLog::LogError(const char *fmt, ...)
{
	...
	if (pF)
	{
		...
		ALERT("[AMXX] Unexpected fatal logging error (couldn't open %s for a+). AMXX Error Logging disabled for this map.\n", file);
		m_FoundError = true;
		return;
	}
